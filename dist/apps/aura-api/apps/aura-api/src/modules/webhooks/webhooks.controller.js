...
let WebhooksController = class WebhooksController {
    ...
    async verifyInstagramWebhook(mode, token, challenge, res) {
        try {
            const returnedChallenge = await this.webhooksService.verifyWebhook(mode, token, challenge);
            res.status(common_1.HttpStatus.OK).send(returnedChallenge);
        }
        catch (error) {
            res.status(common_1.HttpStatus.FORBIDDEN).send('Forbidden');
        }
    }
    async handleInstagramWebhook(body, res) {
...
], WebhooksController.prototype, "verifyInstagramWebhook", null);
__decorate([
    ...
    __metadata("design:returntype", Promise)
], WebhooksController.prototype, "handleInstagramWebhook", null);
